...
>(
  ...
  const formatter = (
    ...
    if (adt.type === rrKey) {
      return R.format(rrMatch.formatter, adt.value as RR);
    }
    if (adt.type === ssKey) {
      return R.format(ssMatch.formatter, adt.value as SS);
    }
    if (adt.type === ttKey) {
      return R.format(ttMatch.formatter, adt.value as TT);
    }
    if (adt.type === uuKey) {
      return R.format(uuMatch.formatter, adt.value as UU);
    }
    if (adt.type === vvKey) {
      return R.format(vvMatch.formatter, adt.value as VV);
    }
    if (adt.type === wwKey) {
      return R.format(wwMatch.formatter, adt.value as WW);
    }
    return R.format(xxMatch.formatter, adt.value as XX);
  }
  return {
